...
function lb_remove_this_page_from_pages( $id ) {
	global $lb_pdo;

	$result = $lb_pdo->prepare( 'DELETE FROM `pages` WHERE id = :id ' );
...
function lb_edit_page( $id, $title, $content ) {
	global $lb_pdo;

	$result = $lb_pdo->prepare( 'UPDATE `pages` SET `name`=:name, `html`=:html WHERE `id` = :id' );
